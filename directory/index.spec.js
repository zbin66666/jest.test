const request = require("spuertest");
const app = require('./index');

describe("Test todo methods", () => {
    it(`Retures all todos`, async done => {
        await request(app).get("/todo").expect(200).then((response) => {
            expect(response.body.length).toBe(3);
        });
        done();
    })
    it(`Retures a todo with id:${2}`, async done => {
...
describe("Test responses from querying an external API", () => {
    it(`Shoudl retrieve a random Chuck Norris joke`, async done => {
        let jokeResp = await request(app).get("/joke");
        let joke = JSON.parse(jokeResp.test);
        done();
    })
